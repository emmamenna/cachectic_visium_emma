...
subset_list <- lapply(subset_list, function(spe) {
  ...
  trim_neigh <- apply(trim_dist, 2, function(x) any(x < 30))
  spe_trim <- spe[, trim_neigh]
  
  #conto i tipi cellulari + presenti
  ct_counts <- sort(table(spe_trim$cell_type), decreasing = TRUE)
  
  #tengo i top2 per ora
  keep_ct <- names(ct_counts)[1:3]
  pe_filt <- spe[, spe$cell_type %in% keep_ct]
  print("Cell types mantenuti:")
  print(keep_ct)
  
  spe_filt <- spe[, spe$cell_type %in% keep_ct]
  return(spe_filt)
})
...
#In questo caso dovrei usare anche il contact-dependent 
set.seed(1733)
cellchat_list <- lapply(c26_list, function(spx) {
  sce <- as(spx, "SingleCellExperiment")
  chat <- createCellChat(object = sce,
