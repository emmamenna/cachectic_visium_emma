...
#Dataset preparation:
Firstly, spots that have zero counts or that were identified by deconvolution as NAs (excpet for block 7 -
which we are not considering anymore in the analysis - these correspond to external connective tissue)
are removed. Secondly, a single-cell normalization is applied and lastly, samples are merged according to
tissue_type (condition). Note that this approach is not ideal because batch effect is not
accounted for (more samples are treated as one sample) and spatial information is not being used.
```{r}
...
#Genes filtering
A gene is kept if and only if it is expressed in at least 5% of spots belonging to the same
cell_type in the same tissue_type.
```{r}
...
Applying cell chat to the merged samples (one condition at a time).
```{r}
...
#Visualization -> plots are saved in pdf files (one per condition).
```{r}
# Loop sui campioni
