...
dot_scaled <- dot_data %>%
  group_by(gene) %>%
  mutate(
    avg_expr_scaled = scale(avg_expr)[,1]  # Z-score per gene
  ) %>%
  ungroup()
...
#Concordanza -> k di cohen
library(psych)
t <- table(expr_pax7>0,expr_Cdkn1a>0)
cohen.kappa(t,n.obs=42537)
t <- table(expr_pax7>0,expr_Cdkn2a>0)
cohen.kappa(t,n.obs=42537)
t <- table(expr_pax7>0,expr_Trp53>0)
cohen.kappa(t,n.obs=42537)
t <- table(expr_pax7>0,expr_Myog>0)
cohen.kappa(t,n.obs=42537)
t <- table(expr_pax7>0,expr_Myf5>0)
cohen.kappa(t,n.obs=42537)
#sono tutte nulle
...
#definisco la popolazione dei MuSC
musc_sham <- sce_sham[,sce_sham$cell_type == "MuSC" & !is.na(sce_sham$cell_type)]
dim(musc_sham)
musc_sham[rowSums(counts(musc_sham))>0,]
dim(musc_sham) #6798 geni espessi in almeno uno spot MuSC sham

expr_pax7 <- as.vector(counts(sce_sham[rownames(sce_sham) == "Pax7",]))
table(expr_pax7>0)
summary(expr_pax7)
expr_Cdkn1a <- as.vector(counts(sce_sham[rownames(sce_sham) == "Cdkn1a",]))
expr_Cdkn2a <- as.vector(counts(sce_sham[rownames(sce_sham) == "Cdkn2a",]))
expr_Trp53 <- as.vector(counts(sce_sham[rownames(sce_sham) == "Trp53",]))
expr_Myog <- as.vector(counts(sce_sham[rownames(sce_sham) == "Myog",]))
expr_Myf5 <- as.vector(counts(sce_sham[rownames(sce_sham) == "Myf5",]))
table(expr_pax7>0,expr_Cdkn1a>0) #solo questo sembra particolarmente correlato, ma sono numeri piccoli
table(expr_pax7>0,expr_Cdkn2a>0)
table(expr_pax7>0,expr_Trp53>0)
table(expr_pax7>0,expr_Myog>0)
