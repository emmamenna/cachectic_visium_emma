...
pb_wb7 <- pb_wb7[,pb_wb7$tissue_type %in% c("c26","sham")]
...
#solo nello sham_b1 per gli 8bin
#Visualizzazione nel tessuto con scala comune per campione (per confronto)
```{r}
load("~/spe_list_8bin.RData")
data <- read.table("~/geni_identita.txt")
vec <- as.vector(data)
markers <- unlist(vec[1], use.names = FALSE)
markers
...
for (i in seq_along(markers)) {
  ...
  # Salva un file per marker
  ggsave(
    filename = paste0("plot_sham_b1" , "_", marker, ".png"),
    plot = p,
    width = 6,
    height = 6,
    dpi = 300
