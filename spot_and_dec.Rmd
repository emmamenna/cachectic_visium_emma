...
run_my_rctd <- function(spe, sce_ref, out_prefix) {
 ...
 your_color_vector <- c(
  ...
  stat1 <- plotCoords(spe, annotate = "cell_type", point_size = 0.7) +
...
ct_long <- lapply(subset_list, function(spe) {
  data.frame(
...
```{r}
prop_per_sample <- lapply(names(subset_list), function(sample_name) {
  spe <- subset_list[[sample_name]]
  df <- data.frame(
    sample = sample_name,
    tissue_type = unique(spe$tissue_type),
