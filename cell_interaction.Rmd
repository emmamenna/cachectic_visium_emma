...
spicyTest$p.value
spicyTest$pairwiseAssoc #weird 

#INDAGARE DI +
#spicy() dopo può standardizzare o pesare quei valori, per cui i coefficienti del modello possono essere positivi anche se le curve individuali sono negative.
...
#   nsim = 1000
# )
# spicyTestBoot$coefficient
# spicyTestBoot$p.value
#stessa cosa
...
inhomLPair_mod <- function (
    ...
    if (is.null(Rs)) Rs = seq(20, 500, by = 100)
...
#NB: eventualmente modificare i valori per il raggio
#tenendo conto che il raggio "totale" (metà range)
#è 6500/8000 per il primo campione e che 
#si tratta di una misura aggregata
#e che sotto i 50 compresi non trova nulla
...
#"normalizzo" L(r) - r così l'asse y=0 diventa la linea di Poisson
for (sample in names(all_Lcurves)) {
  Lcurve <- all_Lcurves[[sample]]
  Lcurve <- Lcurve %>%
    mutate(LminusR = wt - r)
  p <- ggplot(Lcurve, aes(x = r, y = LminusR, color = interaction(cellTypeI, cellTypeJ))) +
    geom_line(size = 1) +
    geom_hline(yintercept = 0, linetype = "dashed", color = "gray50") +
    theme_minimal(base_size = 14) +
    labs(
      title = paste("Curva L(r) - r - Campione:", sample),
      x = "Distanza r",
      y = "L(r) - r",
      color = "Coppia cellulare"
    )
  print(p)
  # ggsave(paste0("Lcurve_", sample, ".png"), plot = p, width = 6, height = 4)
}
