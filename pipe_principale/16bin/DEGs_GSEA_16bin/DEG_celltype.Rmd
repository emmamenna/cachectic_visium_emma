...
# # all_cols <- Reduce(union, lapply(subset_list, function(x) colnames(colData(x))))
# # common_cols <- Reduce(intersect, lapply(subset_list, function(x) colnames(colData(x))))
...
#                                 id=DataFrame(label = combined_spe$new_cell_type,
#                                              sample = combined_spe$sample_id))
# 
# colnames(pb) <- paste(pb$new_cell_type, pb$sample_id, sep = "_")
# saveRDS(pb, file = "~/pb_16bin.rds")
...
#DEG PSEUDOBULK CON SOLI MIONUCLEI
```{r}
subset_list <- readRDS("~/bin16_list_ann.rds")
