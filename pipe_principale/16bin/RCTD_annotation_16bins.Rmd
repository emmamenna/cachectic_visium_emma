...
#Exploratory analysis on 16um bins
```{r}

# Ciclo su tutti i nomi della lista
for (nm in names(subset_list)) {
  valore_blocco <- sub(".*_", "", nm)
  cond <- sub("_.*", "", nm)
  subset_list[[nm]]$blocco <- valore_blocco
  subset_list[[nm]]$tissue_type <- cond
  if ("X" %in% assayNames(subset_list[[nm]])) {
    assayNames(subset_list[[nm]])[assayNames(subset_list[[nm]]) == "X"] <- "counts"
  }
}
names(subset_list) <- names(subset_list)

estrai_info <- lapply(subset_list, function(spe){
    n_spot_iniziale <- ncol(spe)
    total_umi <- sum(colSums(counts(spe)))
    n_geni <- sum(rowSums(counts(spe)) != 0)
    data.frame(
      Condition = unique(spe$tissue_type),
...
#write_xlsx(tabella_finale, path = "expl_analysis_bin16.xlsx")
...
```{r}
prop_df_condition <- lapply(subset_list, function(spe) {
  data.frame(
