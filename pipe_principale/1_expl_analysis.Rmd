...
#Function to read data from Visium HD
A bin size of preference needs to be selected between 002, 008 and 016, and the
correspondent list defining bins in and out of tissue has to be loaded.
```{r, include=FALSE}
...
Filtering data to only keep in-tissue bins and separating the 3 samples in the same block.
```{r}
...
processa_blocco <- function(blocco_num, lista_contissue) {
  ...
  # Optional filtering based on UMIs and genes 
...
#Data without filtering has been saved as speHD_g objects.
...
#Check mithocondrial and ribosomal genes.
```{r}
...
#Recalling a code for plotting all blocks.
```{r child = "Plot_muscoli.Rmd"}
```


#Pseudo-bulk exploratory analysis
Aggregating blocks: let's first create a single-cell object merging all samples,
defining variables to identify tissue_type (condition: c26, sham etc.) and block.
```{r}
...
#let's add a variable to identify samples (combination of block and tissue_type)

```

Then, let's create a pseudo-bulk object merging (sum) all counts from the same sample.
```{r}
...
#PCA
Let's observe how samples divide: there seems to be an aggregation dependent on conditions
more than bathc effect (good).
```{r, include=FALSE}
...
Repeating the same analysis while selecting only the 5000 most variable genes (around 15% of all genes).
```{r, message=FALSE, warning=FALSE}
...
#Summary of samples information (UMIs, observed genes and number of spots)
```{r}
...
#Graphs related to the summary table just found.
```{r}
library(readxl)
...
pb <- aggregateAcrossCells(pb_wb7, ids=pb_wb7$sample_id)
y <- DGEList(counts(pb), samples = colData(pb))

#filtering genes observed in less than 10% of spots
