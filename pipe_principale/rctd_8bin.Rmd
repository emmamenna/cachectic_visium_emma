---
title: "8um bins annotation with Rctd"
author: "Emma Menna"
...
Using "doublets" mode does not change much results in this case.
I set UMI_min to 15, considering that 100 is the recommended value
for Visium non HD which has 55um resolution. 
Therefore 55:8 = 100:x ; x = 15.
```{r}
...
#saving results
saveRDS(spe_list, file = "bin8_list_ann.rds")
```

#Projections on tissue for all samples
For each sample 2 plots are created. In the first, for all cell types spots are coloured according to the probabiluty (weigth) for that cell type. In the second one, each spot is coloured according to its clustering labels (the cell type with highest weigth).
```{r}
